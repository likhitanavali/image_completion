...
import keras.engine.topology as kep
...
def example_gan(result_dir="output", data_dir="data"):
    ...
    completion = merge([imitation, org_img, mask],mode=lambda x: x[0] * x[2] + x[1] * (1 - x[2]),output_shape=input_shape)
    cmp_container = kep.Container([org_img, mask], completion)
    ...
    d_container = kep.Container([org_img, in_pts], discriminator([org_img, in_pts]))
